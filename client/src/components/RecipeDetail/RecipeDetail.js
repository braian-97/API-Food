...
function RecipeDetail({ id, getRecipeDetail, details, errors }) {
    const span = useRef()

    useEffect(() => {
        getRecipeDetail(id);
        if (details && details.summary &&  span.current) {
            span.current.innerHTML = details.summary
        }
    }, [id])
            ...
            {details && typeof details !== 'string' ?
                <div className={s.recipe}>
                    ...
                    <div id="summary" className={s.summaryDetails}>
                        <div className={s.summary} ><h3>Resumen: </h3> <span ref={span}></span></div>
                    </div>
