            ...
            <form onSubmit={handleSubmit} id="addForm">
                ...
                <div>
                    ...
                    <div className="addImg">
                        ...
                        <input placeholder="Url of the image" type="text" name="image" id="image" onChange={handleInputChange} value={recipe.image} size="80" ></input><input type="button" value="Clear" onClick={() => setRecipe({ ...recipe, image: "" })} /> <br></br>
                        <h5>Select Image</h5>
                        <p className="help">Tamaño maximo permitido : 100000 Bytes(97.65 Kilobytes)</p>
                        <input type="file" name="myImage" onChange={onFileChange} /><br></br>
                        {errorImage ? <span className="help"><p><strong>Error : Imagen demaciado grande</strong></p><p>{errorImage}</p></span> : null}
                        <br></br>
                ...
                <div className="addDiet">
                    <div className="listDiet">                     
                        <ul>
                            <h5>Current diets: </h5>
                            {diets.map((d, i) => (
                                <li className="listAddDiet" key={d.id}>
                                    <input className="check" type="checkbox" name="diet" onChange={handleInputChange} value={d.name}></input>
                                    <label className="labelDiet">{d.name}</label>
                                </li>
                            )
                            )}
                        </ul>
                    ...
                    <div>
                        ...
                        <p className="help">When adding more than one new diet, separate the different diets with a ","</p>
                    </div>
                ...
                <button className="addButton" type="submit" disabled={recipe.name !== "" && recipe.summary !== "" ? false : true} >Create recipe</button>
            </form>
            {result && result.id ? <div><h4>Recipe successfully created: </h4><button onClick={routeChange}> Go to recipe </button></div> : null}
            {result && !result.id ? <h4 className="help">Error: recipe creation failed</h4> : null}
        </div>
    )
}
